...
/**
 * Dialog for importing indirect clients (payors) from a JSON file.
 */
public class PayorImportDialog extends Dialog {
    ...
    public PayorImportDialog(PayorEnrolmentService payorEnrolmentService, Runnable onImportComplete) {
        this.payorEnrolmentService = payorEnrolmentService;
        this.onImportComplete = onImportComplete;

        setHeaderTitle("Import Indirect Clients");
        setWidth("700px");
    ...
    private void showUploadStep() {
        ...
        Span instructions = new Span("Upload a JSON file containing indirect client information.");
    ...
    private Anchor createTemplateDownloadLink() {
        ...
        StreamResource resource = new StreamResource("indirect-client-template.json",
    ...
    private void validateFile() {
        ...
        try {
            validationResult = payorEnrolmentService.validateFile(fileContent, fileName);
    ...
    private void showValidationSuccessStep() {
        ...
        Span summary = new Span("Ready to import " + validationResult.getPayorCount() + " indirect clients:");
    ...
    private void showValidationErrorsStep() {
        ...
        Span summary = new Span(validationResult.getErrors().size() + " errors found:");
    ...
    private void startImport() {
        ...
        try {
            payorEnrolmentService.executeBatch(validationResult.getBatchId());
    ...
    private void showProgressStep(String batchId) {
        ...
        startPolling(batchId, progressBar, statusLabel, successCount, failedCount, pendingCount, failedGrid);
    }
    ...
    private void startPolling(String batchId, ProgressBar progressBar, Span statusLabel,
                              ...
                              Grid<BatchItemDto> failedGrid) {
        scheduler = Executors.newSingleThreadScheduledExecutor();
        pollFuture = scheduler.scheduleAtFixedRate(() -> {
            getUI().ifPresent(ui -> ui.access(() -> {
                updateProgress(batchId, progressBar, statusLabel, successCount, failedCount, pendingCount, failedGrid);
                ui.push();
            }));
